			...
			for(var i=0;i<result.data.length;i++){
				...
				<div class="layui-col-md3">
                	<div id="" class="box">
                    	...
                    	<strong>`+result.data[i].sname+`</strong>
                    	<span>发布：`+result.data[i].team.tname+`</span>
                    	<span class="layui-icon layui-icon-user">`+result.data[i].snum+`</span>
                	</div>
            	</div>`;
			}
			$('#science_choose').html(htmltext);
			}
		}
	});
});
...
$('body').on('click',".box",function(){
    ...
    location.href = "scienceinfo.html?sid=" + sid;
})
$('ul').on('click','.scic',function(){
	var type=$(this).text();
	location.href = "sciencechoose?type=" + type;
})
			...
			layui.use('layer', function(){
				var layer = layui.layer;
					layer.msg(result.msg, {
						...
						window.location.href="sciencelist.html";
					})
				})
			}
			}
