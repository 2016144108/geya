	...
	$.ajax({
		...
		success : function(result) {
			if(result.code==200){
				...
				$("#start").val(result.data.sstime);
				$("#end").val(result.data.setime);
...
$('body').on("click",".send",function(){
	...
	if((sname!=""&&sname!=null)&&(uid!=""&&uid!=null)&&(stelephone!=""&&stelephone!=null)&&(sstime!=""&&sstime!=null)&&(setime!=""&&setime!=null)&&(sintro!=""&&sintro!=null)){
		$.ajax({
			url : "../editerservice?sid="+sid+"&sname="+sname+"&uid="+uid+"&stelephone="+stelephone+"&sstime="+sstime+"&setime="+setime+"&sintro="+sintro,
			dataType : "json",
			async : false,
			success : function(result) {
				layui.use('layer', function(){
...
$('body').on("click",".reset",function(){
    window.location.href="serviceediter.html";
})
